...
OUTPUT_CSV = os.getenv("OUTPUT_CSV", "output/apex_hybrid_max_v2_candidates.csv")

MAX_SYMBOLS = int(os.getenv("MAX_SYMBOLS", "0") or "0")

MIN_PRICE = float(os.getenv("MIN_PRICE", "10"))
MIN_MARKET_CAP = float(os.getenv("MIN_MARKET_CAP", "1000000000"))
MIN_DOLLAR_VOLUME_21D = float(os.getenv("MIN_DOLLAR_VOLUME_21D", "12000000"))
...
PULLBACK_MAX_DRAWDOWN_PCT = float(os.getenv("PULLBACK_MAX_DRAWDOWN_PCT", "0.06"))
...
MAX_EXTENSION_FROM_50MA = float(os.getenv("MAX_EXTENSION_FROM_50MA", "0.25"))
GAP_UP_BLOCK_PCT = float(os.getenv("GAP_UP_BLOCK_PCT", "0.05"))
...
INDUSTRY_BREADTH_CUTOFF = float(os.getenv("INDUSTRY_BREADTH_CUTOFF", "45"))
TOP_ETF_COUNT = int(os.getenv("TOP_ETF_COUNT", "5"))
...
FINAL_STOCK_WEIGHT = 85.0
FINAL_ETF_BONUS = 10.0
FINAL_INDUSTRY_BREADTH_BONUS = 5.0
...
class CandidateRow:
    ...
    etf_bonus_score: float
    industry_breadth_bonus: float
...
def assign_theme_etf(sector, industry) -> Optional[str]:
    s = safe_text(sector).upper()
    i = safe_text(industry).upper()
    ...
    if "SOFTWARE" in i:
        return "IGV"
    if "INTERNET" in i or "CONTENT" in i or "ADVERTISING" in i:
        return "FDN"
    if "AEROSPACE" in i or "DEFENSE" in i:
        return "ITA"
    if "MEDICAL DEVICES" in i:
        return "IHI"
    if "METALS" in i or "MINING" in i:
        return "XME"
    if "OIL" in i or "GAS" in i or "ENERGY" in i:
        return "XLE"
    if "AGRICULT" in i or "FARM" in i:
        return "DBA"
    if "ENGINEERING" in i or "MACHINERY" in i or "INDUSTRIAL" in i:
        return "XLI"
    if "INFRASTRUCTURE" in i or "CONSTRUCTION" in i or "BUILDING" in i:
        return "PAVE"
...
def calc_etf_scores(price_map: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    rows: List[Dict] = []

    for etf, name in ETF_UNIVERSE.items():
        df = price_map.get(etf)
        ...
        ret_1m = pct_change(close, close_21)
        ret_3m = pct_change(close, close_63)
        ret_6m = pct_change(close, close_126)

        accel_1 = safe_div(ret_1m, ret_3m / 3.0)
        accel_2 = safe_div(ret_3m, ret_6m / 2.0)

        high_52w = float(df["High"].tail(252).max())
        dist_52w = pct_change(close, high_52w)

        vol_ratio = safe_div(latest(df["Volume"]), float(df["Volume"].tail(20).mean()))

        score = 0.0
        score += score_from_bounds(ret_6m, 5, 40, 30)
        score += score_from_bounds(ret_3m, 3, 25, 25)
        score += score_from_bounds(ret_1m, 1, 12, 15)
        score += score_from_bounds(accel_1, 0.7, 1.8, 10)
        score += score_from_bounds(accel_2, 0.7, 1.6, 5)

        high_prox_metric = max(0.0, 15.0 + dist_52w)
        score += score_from_bounds(high_prox_metric, 0, 15, 10)
        score += score_from_bounds(vol_ratio, 1.0, 2.0, 5)
        ...
        rows.append(
            asdict(
                EtfScoreRow(
                    ...
                    dist_52w_high_pct=round(dist_52w, 2),
    ...
    out = out.sort_values(["total_score", "ret_1m_pct"], ascending=[False, False]).reset_index(drop=True)
    out["rank"] = np.arange(1, len(out) + 1)
    return out


def build_industry_breadth(universe: pd.DataFrame, price_map: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    rows = []

    grouped = universe.groupby("theme_etf")
    for etf, grp in grouped:
        if pd.isna(etf) or not etf:
            continue
        ...
        for ticker in grp["ticker"].tolist():
            df = price_map.get(ticker)
            if df is None or len(df) < 60:
                continue
            ...
            if pd.isna(close) or pd.isna(sma50) or pd.isna(high_52w) or high_52w <= 0:
                continue

            valid += 1
            if close > sma50:
                above += 1
            if close > sma50 and pct_change(close, high_52w) >= -15:
                strong += 1

        if valid == 0:
            breadth = np.nan
            strong_breadth = np.nan
        else:
            breadth = above / valid * 100.0
            strong_breadth = strong / valid * 100.0

        rows.append(
            {
                "theme_etf": etf,
                "industry_breadth_pct": round(breadth, 2) if not pd.isna(breadth) else np.nan,
                "strong_breadth_pct": round(strong_breadth, 2) if not pd.isna(strong_breadth) else np.nan,
                "member_count": valid,
            }
        )
...
def etf_bonus_from_rank(rank: int) -> float:
    if rank == 1:
        return 10.0
    if rank == 2:
        return 8.0
...
def breadth_bonus_from_pct(pct: float) -> float:
    if pd.isna(pct):
        return 0.0
    if pct >= 65:
        return 5.0
...
def calc_setup_type(df: pd.DataFrame) -> Tuple[str, float]:
    ...
    pullback_depth = safe_div(recent_high - recent_low, recent_high)
...
def calc_stock_candidate(
    ...
    accel_1 = safe_div(ret_1m_pct, ret_3m_pct / 3.0)
    accel_2 = safe_div(ret_3m_pct, ret_6m_pct / 2.0)
    ...
    avg_vol_20 = float(volume.tail(20).mean())
    vol_ratio = safe_div(latest(volume), avg_vol_20)
    ...
    trend_quality_raw += 1 if dist_52w_high_pct >= -15 else 0
    ...
    high_prox_metric = max(0.0, 15.0 + dist_52w_high_pct)
    score_high_prox = score_from_bounds(high_prox_metric, 0, 15, W_HIGH_PROX)
    ...
    stock_score_scaled = stock_score_total * (FINAL_STOCK_WEIGHT / 100.0)
    etf_bonus_score = etf_bonus_from_rank(theme_etf_rank)
    industry_breadth_bonus = breadth_bonus_from_pct(industry_breadth_pct)

    final_score = stock_score_scaled + etf_bonus_score + industry_breadth_bonus
    ...
    return CandidateRow(
        ...
        industry=industry,
        theme_etf=theme_etf,
        theme_etf_rank=int(theme_etf_rank),
        theme_etf_score=round(theme_etf_score, 2),
        industry_breadth_pct=round(industry_breadth_pct, 2),
        ...
        stock_score_total=round(stock_score_total, 2),
        ...
        etf_bonus_score=round(etf_bonus_score, 2),
        industry_breadth_bonus=round(industry_breadth_bonus, 2),
    )
...
def build_candidates(universe: pd.DataFrame, price_map: Dict[str, pd.DataFrame], regime: MarketRegime) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    etf_scores = calc_etf_scores(price_map)
    if etf_scores.empty:
        return pd.DataFrame(), etf_scores, pd.DataFrame()

    top_etfs = etf_scores.head(TOP_ETF_COUNT).copy()
    top_etf_set = set(top_etfs["etf"].tolist())

    universe = universe.copy()
    universe["theme_etf"] = universe.apply(lambda r: assign_theme_etf(r["sector"], r["industry"]), axis=1)
    universe = universe[universe["theme_etf"].notna()].copy()

    industry_breadth = build_industry_breadth(universe, price_map)
    if industry_breadth.empty:
        return pd.DataFrame(), etf_scores, industry_breadth

    breadth_map = industry_breadth.set_index("theme_etf")["industry_breadth_pct"].to_dict()
    universe["industry_breadth_pct"] = universe["theme_etf"].map(breadth_map)

    universe = universe[universe["theme_etf"].isin(top_etf_set)].copy()
    universe = universe[universe["industry_breadth_pct"] >= INDUSTRY_BREADTH_CUTOFF].copy()

    etf_score_map = top_etfs.set_index("etf")["total_score"].to_dict()
    etf_rank_map = top_etfs.set_index("etf")["rank"].to_dict()
    ...
    for _, row in universe.iterrows():
        ...
        industry_breadth_pct = row["industry_breadth_pct"]
        ...
        cand = calc_stock_candidate(
            ...
            spy_df=spy_df,
            theme_etf=theme_etf,
            theme_etf_rank=int(etf_rank_map.get(theme_etf, 999)),
            theme_etf_score=float(etf_score_map.get(theme_etf, 0)),
            industry_breadth_pct=float(industry_breadth_pct),
        )
        if cand is None:
            continue

        day_open = latest(df["Open"])
        if not pd.isna(cand.breakout_level) and not pd.isna(day_open):
            if day_open >= cand.breakout_level * (1 + GAP_UP_BLOCK_PCT):
                continue
    ...
    out = pd.DataFrame(rows)
    if out.empty:
        return out, etf_scores, industry_breadth
    ...
    out = out.sort_values(
        by=["setup_rank", "final_score", "ret_1m_pct", "rs_3m_pct"],
    ...
    return out.head(top_n).copy(), etf_scores, industry_breadth
...
def build_summary_message(regime: MarketRegime, candidates: pd.DataFrame, etf_scores: pd.DataFrame) -> str:
    lines: List[str] = []
    lines.append("Apex Hybrid Max v2")
    lines.append(f"시각: {utc_now()}")
    lines.append("")
    lines.append("시장")
    lines.append(f"- 레짐: {regime.note}")
    lines.append(f"- SPY: {regime.spy_close:.2f} / 50MA {regime.spy_sma50:.2f} / 200MA {regime.spy_sma200:.2f}")
    lines.append(f"- QQQ: {regime.qqq_close:.2f} / 200MA {regime.qqq_sma200:.2f}")
    lines.append(f"- Breadth: {regime.breadth_pct:.2f}%")
    lines.append("")

    if not etf_scores.empty:
        lines.append("ETF 상위 5")
        for _, r in etf_scores.head(TOP_ETF_COUNT).iterrows():
            lines.append(
                f"{int(r['rank'])}. {r['etf']}  점수 {r['total_score']:.1f}  "
                f"(6M {r['ret_6m_pct']:.1f}% / 3M {r['ret_3m_pct']:.1f}% / 1M {r['ret_1m_pct']:.1f}%)"
            )
        lines.append("")
    ...
    lines.append(f"후보 수: {len(candidates)}")
    lines.append("")
    ...
    for i in range(limit):
        ...
        lines.append(
            f"최종 {row['final_score']:.1f} | 종목 {row['stock_score_total']:.1f} | "
            f"ETF {row['theme_etf']}({int(row['theme_etf_rank'])}) | 산업B {row['industry_breadth_pct']:.1f}%"
        )
        lines.append(f"셋업 {row['setup_type']}")
        lines.append(
            f"진입 {row['entry_price']:.2f} | 손절 {row['stop_price']:.2f} | "
            f"+8% {row['pyramid_1_price']:.2f} | +16% {row['pyramid_2_price']:.2f}"
        )
        lines.append(
            f"세부  6M {row['score_ret_6m']:.1f}/20 | 3M {row['score_ret_3m']:.1f}/15 | "
            f"1M {row['score_ret_1m']:.1f}/10"
        )
        lines.append(
            f"가속  1M {row['score_accel_1']:.1f}/15 | 3M {row['score_accel_2']:.1f}/10"
        )
        lines.append(
            f"위치  고점 {row['score_high_prox']:.1f}/15 | RS {row['score_rs']:.1f}/10 | "
            f"거래량 {row['score_volume']:.1f}/5 | 차트 {row['score_trend_quality']:.1f}/10"
        )
        lines.append(
            f"보너스  ETF {row['etf_bonus_score']:.1f}/10 | 산업B {row['industry_breadth_bonus']:.1f}/5"
        )
...
def main() -> None:
    ...
    all_tickers = sorted(set(base_tickers + list(ETF_UNIVERSE.keys()) + universe["ticker"].tolist()))
    ...
    candidates, etf_scores, industry_breadth = build_candidates(universe, price_map, regime)

    save_output(candidates, OUTPUT_CSV)

    message = build_summary_message(regime, candidates, etf_scores)
    print(message)
    send_telegram_message(message)

    print("")
    print(f"[INFO] ETF scored: {len(etf_scores)}")
    print(f"[INFO] Industry breadth rows: {len(industry_breadth)}")
