...
def build_summary_message(regime: MarketRegime, candidates: pd.DataFrame, etf_scores: pd.DataFrame) -> str:
    ...
    if not etf_scores.empty:
        ...
        for _, r in etf_scores.head(TOP_ETF_COUNT).iterrows():
            lines.append(
                f"- {r['rank']}. {r['etf']} | 점수 {r['total_score']:.1f} | "
    ...
    for i in range(limit):
        ...
        lines.append(
            f"{i+1}. {row['ticker']} {row['name']} | 최종 {row['final_score']:.1f} | "
            f"종목 {row['stock_score_total']:.1f} | ETF {row['theme_etf']}({int(row['theme_etf_rank'])}) | "
            f"산업B {row['industry_breadth_pct']:.1f}% | 셋업 {row['setup_type']}"
        )
        lines.append(
            f"   점수: 6M {row['score_ret_6m']:.1f}/20 | 3M {row['score_ret_3m']:.1f}/15 | "
            f"1M {row['score_ret_1m']:.1f}/10 | 가속1 {row['score_accel_1']:.1f}/15 | "
            f"가속2 {row['score_accel_2']:.1f}/10"
        )
        lines.append(
